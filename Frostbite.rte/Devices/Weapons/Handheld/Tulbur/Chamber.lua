function Create(self)
	...
	self.preSound = CreateSoundContainer("Pre Tulbur", "Frostbite.rte");
	
	self.addSound = CreateSoundContainer("Add Tulbur", "Frostbite.rte");
	self.addAutoSound = CreateSoundContainer("AddAuto Tulbur", "Frostbite.rte");
	
	self.activeAddSound = self.addSound;
	
	self.reflectionOutdoorsSound = CreateSoundContainer("ReflectionOutdoors Tulbur", "Frostbite.rte");
	self.reflectionIndoorsSound = CreateSoundContainer("ReflectionIndoors Tulbur", "Frostbite.rte");
	
	self.reloadPrepareSounds = {}
	self.reloadPrepareSounds.MagOut = CreateSoundContainer("MagOutPrepare Tulbur", "Frostbite.rte");
	self.reloadPrepareSounds.MagIn = CreateSoundContainer("MagInPrepare Tulbur", "Frostbite.rte");
	self.reloadPrepareSounds.MagHit = CreateSoundContainer("MagHitPrepare Tulbur", "Frostbite.rte");
	...
	self.reloadPrepareLengths.MagIn = 670
	self.reloadPrepareLengths.MagHit = 465
	...
	self.reloadPrepareDelay.MagIn = 750
	self.reloadPrepareDelay.MagHit = 650
	self.reloadPrepareDelay.BoltBack = 400
	self.reloadPrepareDelay.BoltForward = 250
	
	self.reloadAfterSounds = {}
	self.reloadAfterSounds.MagOut = CreateSoundContainer("MagOut Tulbur", "Frostbite.rte");
	self.reloadAfterSounds.MagIn = CreateSoundContainer("MagIn Tulbur", "Frostbite.rte");
	self.reloadAfterSounds.MagHit = CreateSoundContainer("MagHit Tulbur", "Frostbite.rte");
	self.reloadAfterSounds.BoltBack = CreateSoundContainer("BoltBack Tulbur", "Frostbite.rte");
	self.reloadAfterSounds.BoltForward = CreateSoundContainer("BoltForward Tulbur", "Frostbite.rte");
	
	self.reloadAfterDelay = {}
	self.reloadAfterDelay.MagOut = 500
	self.reloadAfterDelay.MagIn = 400
	self.reloadAfterDelay.MagHit = 200
	self.reloadAfterDelay.BoltBack = 250
...
function Update(self)
	...
	if self:IsReloading() then
		...
		elseif self.reloadPhase == 2 then
			...
			self.reloadDelay = self.reloadPrepareDelay.MagHit;
			self.afterDelay = self.reloadAfterDelay.MagHit;		
			
			self.prepareSound = self.reloadPrepareSounds.MagHit;
			self.prepareSoundLength = self.reloadPrepareLengths.MagHit;
		...
		if self.reloadTimer:IsPastSimMS(self.reloadDelay) then
			...
			if self.afterSoundPlayed ~= true then
				...
				if self.reloadPhase == 0 then
					self.phaseOnStop = 1;
					local fake
					fake = CreateMOSRotating("Fake Magazine MOSRotating Tulbur");
	...
	if self.FiredFrame then
		...
		local shot = CreateMOPixel("Pellet Tulbur Extra", "Frostbite.rte");
		...
		shakenessParticle.Lifetime = 400;
