function Create(self)
	...
	self.preSound = CreateSoundContainer("Pre Adjudicator", "Frostbite.rte");
	
	self.addSound = CreateSoundContainer("Add Adjudicator", "Frostbite.rte");
	self.addAutoSound = CreateSoundContainer("AddAuto Adjudicator", "Frostbite.rte");
	self.activeAddSound = self.addSound
	
	self.noiseOutdoorsSound = CreateSoundContainer("NoiseOutdoors Adjudicator", "Frostbite.rte");
	self.noiseIndoorsSound = CreateSoundContainer("NoiseIndoors Adjudicator", "Frostbite.rte");
	
	self.reflectionOutdoorsSound = CreateSoundContainer("ReflectionOutdoors Adjudicator", "Frostbite.rte");
	self.reflectionIndoorsSound = CreateSoundContainer("ReflectionIndoors Adjudicator", "Frostbite.rte");
	
	self.reloadPrepareSounds = {}
	self.reloadPrepareSounds.MagOut = CreateSoundContainer("MagOutPrepare Adjudicator", "Frostbite.rte");
	self.reloadPrepareSounds.MagIn = CreateSoundContainer("MagInPrepare Adjudicator", "Frostbite.rte");
	self.reloadPrepareSounds.BoltBack = nil;
	self.reloadPrepareSounds.BoltForward = nil;
	
	self.reloadPrepareLengths = {}
	self.reloadPrepareLengths.MagOut = 1050
	self.reloadPrepareLengths.MagIn = 1380
	self.reloadPrepareLengths.BoltBack = 0
	self.reloadPrepareLengths.BoltForward = 0
	
	self.reloadPrepareDelay = {}
	self.reloadPrepareDelay.MagOut = 1050
	self.reloadPrepareDelay.MagIn = 1900
	self.reloadPrepareDelay.BoltBack = 230
	self.reloadPrepareDelay.BoltForward = 50
	
	self.reloadAfterSounds = {}
	self.reloadAfterSounds.MagOut = CreateSoundContainer("MagOut Adjudicator", "Frostbite.rte");
	self.reloadAfterSounds.MagIn = CreateSoundContainer("MagIn Adjudicator", "Frostbite.rte");
	self.reloadAfterSounds.BoltBack = CreateSoundContainer("BoltBack Adjudicator", "Frostbite.rte");
	self.reloadAfterSounds.BoltForward = CreateSoundContainer("BoltForward Adjudicator", "Frostbite.rte");
	
	self.reloadAfterDelay = {}
	self.reloadAfterDelay.MagOut = 700
	self.reloadAfterDelay.MagIn = 300
	self.reloadAfterDelay.BoltBack = 320
	self.reloadAfterDelay.BoltForward = 700
	...
	self.recoilStrength = 22 -- multiplier for base recoil added to the self.recoilStr when firing
...
function Update(self)
	...
	if self:IsReloading() then
		...
		if self.reloadPhase == 0 then
			...
			self.reloadSupportOffsetTarget = Vector(1, 5)
		...
		elseif self.reloadPhase == 1 then
			...
			self.reloadSupportOffsetTarget = Vector(-2, 13)
			...
			self.reloadingVectorTarget = Vector(-2, 0);
		...
		elseif self.reloadPhase == 2 then
			...
			self.reloadingVectorTarget = Vector(0, -1);
		...
		if self.BaseReloadTimer:IsPastSimMS(self.reloadDelay) then
			...
			if self.reloadPhase == 0 then
				...
				if self.BaseReloadTimer:IsPastSimMS(self.reloadDelay + ((self.afterDelay/5)*2)) then
					self.reloadSupportOffsetTarget = Vector(2, 9)
				else
					self.reloadSupportOffsetTarget = Vector(3, 10)
				end
			...
			elseif self.reloadPhase == 2 then
				...
				self.reloadSupportOffsetTarget = Vector(-1, -1)
				if self.BaseReloadTimer:IsPastSimMS(self.reloadDelay + ((self.afterDelay/5)*3)) then
					self.Frame = 4;
					self.reloadSupportOffsetSpeed = 16
					self.reloadSupportOffsetTarget = Vector(-6, -1)
			...
			elseif self.reloadPhase == 3 then
				...
				elseif self.BaseReloadTimer:IsPastSimMS(self.reloadDelay + ((self.afterDelay/5)*1.25)) then
					self.Frame = 1;
				elseif self.BaseReloadTimer:IsPastSimMS(self.reloadDelay + ((self.afterDelay/5)*1)) then
					self.Frame = 2;
			...
			if self.afterSoundPlayed ~= true then
				...
				if self.reloadPhase == 0 then
					self.phaseOnStop = 1;
					local fake
					fake = CreateMOSRotating("Fake Magazine MOSRotating Adjudicator");
	...
	if self.parent and self.delayedFirstShot == true then
		...
		if fire then
			if not self.Magazine or self.Magazine.RoundCount < 1 then
				--self:Reload()
				self:Activate()
			elseif not self:IsReloading() and not self.activated and not self.delayedFire and self.fireDelayTimer:IsPastSimMS(1 / (self.RateOfFire / 60) * 1000) then
				self.activated = true
	...
	elseif self.autoFiring and not fire then
